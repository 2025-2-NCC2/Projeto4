...
db.connect(err => {
    ...
    console.log('✅ Backend conectado à base de dados MySQL com sucesso!');
});
...
app.get('/api/equipes', (req, res) => {
    ...
    db.query(sql, (err, data) => {
        if (err) return res.status(500).json({ error: "Erro ao buscar equipes." });
...
app.get('/api/transacoes-alimentos', (req, res) => {
    ...
    `;
    db.query(sql, (err, data) => {
        if (err) return res.status(500).json({ error: "Erro ao buscar transações de alimentos." });
        res.json(data);
    });
});

// Outros GETs (mantidos por brevidade)
...
// CORREÇÃO: Recebe e guarda o ID da campanha.
app.post('/api/doacoes', (req, res) => {
    const { valor, data, doador, modo, equipe, campanha_id } = req.body; // <-- campanha_id adicionado
...
app.post('/api/transacoes-alimentos', (req, res) => {
    const { tipo, item, qtd, equipe, destino, nomeNotaFiscal, campanha_id } = req.body; // <-- campanha_id adicionado
    db.query("SELECT item_id FROM Estoque_Itens WHERE nome = ?", [item], (err, itemResults) => {
        ...
        } else { // Saída (não associada a campanha)
...
// Outros POSTs (mantidos para retornar o objeto criado)
